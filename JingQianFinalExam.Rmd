---
title: "JingQianFinalExamination"
runtime: shiny
...
### Question 2:. 5 is a __Integer__ datatype.

### Question 3: "" is an __Null__ datatype.
...
### Question 7: When we run the command *j<-c("d",1,"b")* in *R*, creates a  __array__ stored in *j*.
...
```{r}
median_Sepal.Width <- median(mydata$Sepal.Width)
print(median_Sepal.Width)
```
...
```{r}
p <- ggplot(mydata, aes(y = Sepal.Width, x = Species)) + geom_violin()
print(p)
```
...
```{r}
myanswer <- "The MIT License gives this permission under the stipulation that the License is distributed with that software, while the GPL License only grants use and modification by non-proprietary software."
```
...
### Question 14. Write an R codeblock that replaces "MIT" with "Massachusetts Institute of Technology" and "GPL" with "General Public License" within "myanswer"

```{r}
gsub('MIT', 'Massachusetts Institute of Technology', myanswer)
gsub('GPL', 'General Public License', myanswer)
```
